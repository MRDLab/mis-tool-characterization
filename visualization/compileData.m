function [data, key] = compileData(folderName,freqArray,numTorque,USE_LEFT_JAW)
% File to compile the data
[ key ] = generateKey();

% Parameters to run the file
data = cell(length(freqArray)*(numTorque - 1));

for ii = 1:length(freqArray)
    for jj = 0:numTorque - 1
        % Make the filename
        filename = sprintf('%s/%d%d.TXT',folderName,freqArray(ii),jj);
        ...
        if isOctave == 0
            ...
            % Octave doesn't support the fdesign function, so don't
            % generate filtered values or derivatives
            for kk=1:key.c.NumCols
                % Simple Derivative (i.e. - velocity)
                unitdata(:,kk,key.r.diff) = gradient(unitdata(:,kk,key.r.raw))*1e3;          % Convert rad/ms to rad/s
            end
        ...
        % Store it
        data{numTorque*(ii-1)+jj+1} = unitdata;
    end
end